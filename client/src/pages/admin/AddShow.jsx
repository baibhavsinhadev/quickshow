...
const AddShow = () => {
    ...
    // Handle Date Time Remove Function
        ...
        <>
                            ...
                            <div className="relative rounded-lg overflow-hidden">
                                ...
                                <div className="text-sm flex items-center justify-between p-2 bg-black/70 w-full absolute bottom-0 left-0">
                                    <p className="flex items-center gap-1 text-gray-400">
                                        ...
                                        {playingMovie.vote_average.toFixed(1)}
                                    </p>

                                    <p className="text-gray-300">{formatNumber(playingMovie.vote_count)} Votes</p>
                                </div>
                            </div>

                            {selectedMovie === playingMovie.id && (
                                <div className="absolute top-2 right-2 flex items-center justify-center bg-primary h-6 w-6 rounded">
                                    <CheckIcon className="w-4 h-4 text-white" strokeWidth={2.5} />
                                </div>
                            )}

                            <p className="font-medium truncate">{playingMovie.title}</p>
                            <p className="text-gray-400 text-sm">{playingMovie.release_date}</p>
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className="mt-8">
                ...
                <div className="inline-flex items-center gap-2 border border-gray-600 px-3 py-2 rounded-md">
                    ...
                    <input min={0} type="number" value={showPrice} onChange={(e) => setShowPrice(e.target.value)} placeholder="Enter show price" className="outline-none number-input" />
                </div>
            </div>
            ...
            <div className="mt-6">
                ...
                <div className="inline-flex gap-5 border border-gray-600 p-1 pl-3 rounded-lg">
                    <input type="datetime-local" value={dateTimeInput} onChange={(e) => setDateTimeInput(e.target.value)} placeholder="Enter show price" className="outline-none rounded-md calender-input" />

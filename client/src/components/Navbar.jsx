    ...
    return (
        <div className="fixed top-0 left-0 z-50 w-full flex items-center justify-between px-6 md:px-16 lg:px-36 py-5">
            ...
            <div className={`max-md:absolute max-md:top-0 max-md:left-0 max-md:font-medium max-md:text-lg z-50 flex flex-col md:flex-row items-center max-md:justify-center gap-8 md:px-8 py-3 max-md:h-screen md:rounded-full backdrop-blur bg-black/70 md:bg-white/10 md:border border-gray-300/20 overflow-hidden transition-[width] duration-300 ${isOpen ? "max-md:w-full" : "max-md:w-0"}`}>
                ...
                <NavLink onClick={() => { setIsOpen(!isOpen); scrollTo(0, 0) }} className={({ isActive }) => `transition duration-200 hover:text-primary ${isActive ? "text-primary border-b-2 border-primary pb-1" : ""}`} to="/">
                    Home
                </NavLink>

                <NavLink onClick={() => { setIsOpen(!isOpen); scrollTo(0, 0) }} className={({ isActive }) => `transition duration-200 hover:text-primary ${isActive ? "text-primary border-b-2 border-primary pb-1" : ""}`} to="/movies">
                    Movies
                </NavLink>

                <NavLink onClick={() => { setIsOpen(!isOpen); scrollTo(0, 0) }} className={({ isActive }) => `transition duration-200 hover:text-primary ${isActive ? "text-primary border-b-2 border-primary pb-1" : ""}`} to="/favorite">
                    Favorites
                </NavLink>

                <NavLink onClick={() => { setIsOpen(!isOpen); scrollTo(0, 0) }} className={({ isActive }) => `transition duration-200 hover:text-primary ${isActive ? "text-primary border-b-2 border-primary pb-1" : ""}`} to="/admin">
                    Admin
                </NavLink>
            </div>
